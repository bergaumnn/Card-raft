        ...
        <div className="w-80 flex flex-col justify-center px-8 space-y-4">
          {data.phone && (
            <div className="flex items-start gap-3">
              <Phone className="w-5 h-5 flex-shrink-0 mt-0.5" style={{ color: primaryColor }} />
              <span className="text-base font-sans text-gray-700 leading-tight">{data.phone}</span>
            </div>
          )}
          {data.email && (
            <div className="flex items-start gap-3">
              <Mail className="w-5 h-5 flex-shrink-0 mt-0.5" style={{ color: primaryColor }} />
              <span className="text-base font-sans text-gray-700 break-all leading-tight">{data.email}</span>
            </div>
          )}
          {data.website && (
            <div className="flex items-start gap-3">
              <Globe className="w-5 h-5 flex-shrink-0 mt-0.5" style={{ color: primaryColor }} />
              <span className="text-base font-sans text-gray-700 leading-tight">{data.website}</span>
            </div>
          )}
          {data.address && (
            <div className="flex items-start gap-3">
              <MapPin className="w-5 h-5 flex-shrink-0 mt-0.5" style={{ color: primaryColor }} />
              <span className="text-base font-sans text-gray-700 leading-tight">{data.address}</span>
            </div>
          )}
        </div>
      </div>
